...
class DiffusionInferer(Inferer):
    ...
    def __call__(
        ...
        if mode == "concat":
            ...
            condition = None
        prediction = diffusion_model(x=noisy_image, timesteps=timesteps, class_labels=label, context=condition)
        ...
        for t in progress_bar:
            ...
            if mode == "concat":
                ...
                model_output = diffusion_model(
                    model_input, timesteps=torch.Tensor((t,)).to(input_noise.device), class_labels=label, context=None
                )
            else:
                model_output = diffusion_model(
                    image, timesteps=torch.Tensor((t,)).to(input_noise.device), class_labels=label, context=conditioning
                )
...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
        ...
        """
        with torch.no_grad():
            latent_input = autoencoder_model.encode_stage_2_inputs(inputs) * self.scale_factor[0]
            latent_condition = autoencoder_model.encode_stage_2_outputs(condition) * self.scale_factor[1]
    ...
    def sample(
        ...
        Args:
            ...
            verbose: if true, prints the progression bar of the sampling process.
        """
        latent_condition = autoencoder_model.encode_stage_2_outputs(conditioning) * self.scale_factor[1]
        ...
        image = autoencoder_model.decode_stage_2_outputs(latent / self.scale_factor[0])

        if save_intermediates:
            intermediates = []
            for latent_intermediate in latent_intermediates:
                intermediates.append(autoencoder_model.decode_stage_2_outputs(latent_intermediate / self.scale_factor[0]))
            return image, intermediates
